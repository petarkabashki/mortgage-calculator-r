...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      numericInput("balance", "Mortgage amount:", min = 5000, value=500000),
      ...
      # numericInput("over", "Regular monthly overpayment:", min=0, value = 100)
    ),
...
server <- function(input, output) {
  ...
  aDFyearR <- reactive({
    mortgage(input$balance, input$apr, input$years)
  });
